  ...
  return (
    <header className="h-16 border-b border-slate-200 bg-white dark:bg-slate-900 dark:border-slate-800 flex items-center justify-between px-6 z-10 shrink-0 relative">
      ...
      <div className="flex items-center gap-4">
        {/* Active WhatsApp direct link to Carlos Rodriguez Kobe (+34 637 23 71 00) */}
        <a
          ...
          className="p-1.5 rounded-full text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 transition-all flex items-center gap-1 text-[11px] font-extrabold"
          title="Soporte Directo WhatsApp - Carlos Rodriguez Kobe"
        >
          <PhoneCall className="h-4 w-4 shrink-0 text-emerald-500" />
          <span className="hidden sm:inline">Carlos Kobe (+34 637 23 71 00)</span>
        </a>

        {/* Quick Login simulation buttons requested by the user */}
        <div className="hidden xl:flex items-center gap-2 border-l border-slate-200 dark:border-slate-800 pl-4">
          <button
            onClick={() => {
          ...
          <button
            onClick={() => {
              switchRole("player");
              alert("Sesión simulada iniciada para el Jugador (Facundo Campazzo)");
            }}
            className="px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-300 text-[10px] font-extrabold tracking-wide uppercase transition-all flex items-center gap-1"
            title="Iniciar sesión simulada para Usuarios generales"
          >
